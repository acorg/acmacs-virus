...
#define IVR "(IVR)[\\-\\s]*(\\d+[A-Z]*)\\b"
    ...
    static const std::array normalize_data{
        look_replace2_t{std::regex(PREFIX_1 NYMC, std::regex::icase), "NYMC-$1", "$` $'"},

        look_replace2_t{std::regex(PREFIX_1 IDCDC, std::regex::icase), "RG-$1", "$` $'"},

        look_replace2_t{std::regex(PREFIX_1 NIB, std::regex::icase), "NIB-$1", "$` $'"},

        look_replace2_t{std::regex(PREFIX_1 CBER, std::regex::icase), "CBER-$1", "$` $'"},

        look_replace2_t{std::regex(PREFIX_1 IVR, std::regex::icase), "$1-$2", "$` $'"},
