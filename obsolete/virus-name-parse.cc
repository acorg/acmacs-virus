...
acmacs::virus::v2::parse_result_t acmacs::virus::v2::parse_name(std::string_view source, parse_name_f flags, acmacs::debug dbg)
{
    ...
    const auto make_extra = [](const std::smatch& match) { return string::join(acmacs::string::join_space, {acmacs::string::strip(match.prefix().str()), acmacs::string::strip(match.suffix().str())}); };
    ...
    try {
        if (std::smatch match_general_AB_noisy_infix; std::regex_search(source_u, match_general_AB_noisy_infix, re_flu_name_general_AB_noisy_infix)) {
            ...
            name_data = name_data_t{name_t(string::join(acmacs::string::join_slash, fields)), host_t{fields[1]}, loc.country, loc.continent};
...
name_data_t isolation_with_location(const std::smatch& match, acmacs::virus::v2::parse_name_f flags, std::string_view name, std::vector<acmacs::virus::v2::name::parsing_message_t>& messages)
{
    ...
    if (auto location = fix_location(acmacs::string::concat(match[3].str(), ' ', match[4].str()), flags & parse_name_f::lookup_location, nullptr); !location.name.empty()) {
        const auto isolation = match[5].str();
        if (isolation[0] == '-' || isolation[0] == '_' || isolation[0] == ' ')
            return {name_t(acmacs::string::join(acmacs::string::join_slash, match[1].str(), match[2].str(), location.name, isolation.substr(1), year)), host_t{match[2].str()}, location.country, location.continent};
        else
            return {name_t(acmacs::string::join(acmacs::string::join_slash, match[1].str(), match[2].str(), location.name, isolation, year)), host_t{match[2].str()}, location.country, location.continent};
    }
    else {
        ...
        if (!location.name.empty()) {
            return {name_t(acmacs::string::join(acmacs::string::join_slash, match[1].str(), match[2].str(), location.name, acmacs::string::concat(match[4].str(), match[5].str()), year)), host_t{match[2].str()}, location.country, location.continent};
        }
            ...
            location = fix_location(acmacs::string::concat(match[4].str(), match[5].str()), flags & parse_name_f::lookup_location, &messages);
            messages.emplace_back(acmacs::virus::v2::name::parsing_message_t::isolation_absent, match[0].str());
            return {name_t(acmacs::string::join(acmacs::string::join_slash, match[1].str(), match[3].str(), location.name, std::string{"UNKNOWN"}, year)), host_t{match[3].str()}, location.country, location.continent};
        }
        else {
            messages.emplace_back(acmacs::virus::v2::name::parsing_message_t::location_not_found, match.str(3));
            throw parse_name_error{};
...
name_data_t general(const std::smatch& match, acmacs::virus::v2::parse_name_f flags, std::string_view name, std::vector<acmacs::virus::v2::name::parsing_message_t>& messages)
{
    ...
    if (auto host = match[2].str(); !host.empty()) {
        auto location = fix_location(acmacs::string::concat(host, ' ', match[3].str()), flags & parse_name_f::lookup_location, nullptr);
        ...
        if (!location.name.empty()) { // Lyon/CHU -> Lyon CHU
            ...
            if (location.name.empty()) {
                ...
                if (location.name.empty()) {
                    ...
                    host.clear();
                    isolation = acmacs::string::join(acmacs::string::join_dash, match[3].str(), isolation);
                }
            }
        }
        return {name_t(acmacs::string::join(acmacs::string::join_slash, match[1].str(), host, location.name, isolation, fix_year(match[5].str(), name, &messages))), host_t{host}, location.country, location.continent};
    }
    else { //
        const auto location = fix_location(match[3].str(), flags & parse_name_f::lookup_location, &messages);
        return {name_t(acmacs::string::join(acmacs::string::join_slash, match[1].str(), host, location.name, match[4].str(), fix_year(match[5].str(), name, &messages))), host_t{host}, location.country, location.continent};
    }
...
name_data_t location_not_found(const std::smatch& match, std::string_view name, std::vector<acmacs::virus::v2::name::parsing_message_t>& messages)
{
    ...
    if (match.size() == 6)
        return {acmacs::virus::name_t(acmacs::string::join(acmacs::string::join_slash, match[1].str(), match[2].str(), match[3].str(), fix_isolation(match[4].str()), fix_year(match[5].str(), name, &messages))), acmacs::virus::host_t{match[2].str()}};
    else
        return {acmacs::virus::name_t(acmacs::string::join(acmacs::string::join_slash, match[1].str(), match[2].str(), match[3].str(), std::string{"UNKNOWN"}, fix_year(match[4].str(), name, &messages))), acmacs::virus::host_t{match[2].str()}};
...
location_t fix_location(std::string_view source, acmacs::virus::v2::parse_name_f flags, std::vector<acmacs::virus::v2::name::parsing_message_t>* messages)
{
    ...
    if (const auto found = acmacs::locationdb::get().find(source, acmacs::locationdb::include_continent::yes); found.has_value()) {
        return {found->name, std::string{found->country()}, found->continent};
    }
    else {
        if (messages) {
            messages->emplace_back(acmacs::virus::v2::name::parsing_message_t::location_not_found, source);
